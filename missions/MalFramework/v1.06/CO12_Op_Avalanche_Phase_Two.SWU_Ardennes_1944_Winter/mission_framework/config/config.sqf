...
MF_var_end_civ_cas_enabled = false;  // Mission fail if the friendly casualty rate is too high
MF_var_end_civ_cas_rate = 50;  // Casualty rate in percentage (50 = 50% of the civilians are dead)

// End condition: tasks
MF_var_end_task_enabled = false;  // Mission success if the minimum number of tasks were completed (in any order)
...
MF_var_end_ex_marker = "ext";  // Name of the area marker used for extraction
MF_var_end_ex_threshold = 75;  // 50% of the players should be inside the exfil marker to trigger the mission end
MF_var_end_task_threshold = 100;  // 66% of the tasks should be completed to be a mission success

// Respawn tickets
MF_var_respawn_tickets = -1;  // How many times the player can respawn (-1 = disabled)

// Body removal upon respawn
MF_var_body_removal = true;  // If the players' corpses should be deleted upon respawn

// View distances
MF_var_view_distance_server = 1000;  // Server view distance (= AI view distance)
MF_var_view_distance_player = 1000;  // Player view distance (default for everyone, can be overwritten for individual units in the player init)

// Time acceleration
MF_var_time_acceleration = 0.1;  // Time acceleration between 0.1 - 120 (Note: this has no effect in local hosted sessions)
...
// INTRO TEXT
MF_var_title = "Op. Avalanche - Phase Two";
MF_var_date = "0735 December 28th, 1944";
MF_var_location = "Ardennes, North France";
...
MF_var_rp_tent = "Land_I44_Object_Tent_Zeltbahn_8";  // Custom tent (or any object) used as the Rally Point
