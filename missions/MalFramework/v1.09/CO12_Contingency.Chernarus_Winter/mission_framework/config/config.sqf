...
EGVAR(end_conditions,civilianCasLimit) = 60;  // Percentage of the max. allowed civilian casualty
...
EGVAR(end_conditions,playerThreshold) = 80;  // Percentage of the alive players that have to be in the extraction zone

// Task threshold (used by Time limit, Extraction and Tactical Withdrawal)
GVARMAIN(taskThreshold) = 50;  // Percentage of the tasks that have to be completed
...
EGVAR(intro_text,title) = "CONTINGENCY";  // Title
EGVAR(intro_text,date) = "0615 February 7th, 2016";  // Date
EGVAR(intro_text,location) = "3.5km NE of Elektrozavodsk, Chernarus";  // Location
...
EGVAR(snowfall,maxDensity) = 15;  // Intensity of the snowfall
...
EGVAR(supply_drop,supplyDropPlane) = "UK3CB_BAF_Wildcat_AH1_TRN_8A";  // Plane or helicopter used for the supply drop
