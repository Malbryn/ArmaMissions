// Clickable link to marker:
// <marker name='nameOfTheMarker'>Click here</marker>

// Insert image:
// <img image='mission_framework\config\img\loading_screen.jpg' width='270' height='378'/>

NEWTAB("I. Organization")
<br/><font color='#21749c' size='18' face='PuristaBold'>BLUFOR ORBAT</font>
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>SWEDISH ARMY</font>
<br/>1 SECTION (8x men)
<br/>2 SECTION (8x men)
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>SWEDISH AIR FORCE</font>
<br/>JTAC (2x men)
<br/>LANCER (1x jet)
ENDTAB;

NEWTAB("II. Situation")
<br/><font color='#21749c' size='18' face='PuristaBold'>SUMMARY</font>
<br/>Swedish forces are currently stationed in Chernarus to assist the local CDF troops with fighting against the Chedakis.
<br/>Your platoon is currently stationed at the NW Airfield along with your air force.
<br/>There have been reports of Chedaki forces closing in on the airfield. They have damaged the radar system and set up Grad launchers in the nearby town.
<br/>Your platoon has been alerted and tasked with repelling the enemy attack.
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>AREA OF OPERATIONS</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Terrain</font>
<br/>Hilly terrain with big open fields and thick forest patches.
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Weather</font>
<br/><font size='14' face='PuristaMedium'>Current:</font>
<br/> - Wind: light breeze
<br/> - Precipitation: snowfall
<br/> - Sky: partly cloudy
<br/> - Fog: light fog
<br/>
<br/><font size='14' face='PuristaMedium'>Forecast:</font>
<br/> - No change in weather was forecasted
<br/>
<br/><font size='14' face='PuristaMedium'>Visibility:</font>
<br/> - Approx. view distance: 1km
<br/> - Daytime operation
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Civilian presence</font>
<br/> - Civilian presence is possible inside towns
<br/> - We do care about their lives and properties. Excessive civilian casualties will not be tolerated.
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>ENEMY FORCES</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Composition and Strength</font>
<br/>We are fighting regular Chedaki insurgent forces. They use various Soviet and Russian weaponry. They also utilise armour like BTRs, BMPs and early T-72 variants. We've also spotted unarmed transport helicopters in the region as well. Intel suggests that they have no AA capability.
<br/>We expect enemy QRFs coming from S and SE mostly.
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Capabilities:</font>
<br/><font size='14' face='PuristaMedium'>Fire support:</font>
<br/> - They have Grad rocket artillery system set up in the town of <marker name='mrk_stary'>Stary Sobor</marker>.
<br/> - We have no intel on enemy mortar positions.
<br/>
<br/><font size='14' face='PuristaMedium'>Explosives:</font>
<br/> - Unlikely that they use mines, IEDs or other explosives.
ENDTAB;

NEWTAB("III. Mission:")
<br/><font color='#21749c' size='18' face='PuristaBold'>MISSION INTENT</font>
<br/>Your platoon has been tasked with pushing back the Chedaki troops and capturing stategic locations.
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>OBJECTIVES</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'><marker name='mrk_stary'>At Stary Sobor</marker>:</font>
<br/> - Destroy the Grad launchers and secure the town
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'><marker name='mrk_novy'>At Novy Sobor</marker>:</font>
<br/> - Eliminate enemy forces and secure the town
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'><marker name='mrk_pop'>At Pop Ivan</marker>:</font>
<br/> - Secure the radar station
ENDTAB;

NEWTAB("IV. Execution:")
<br/>Up to Platoon Commander
ENDTAB;

NEWTAB("V. Support & Logistics:")
<br/><font color='#21749c' size='18' face='PuristaBold'>SUPPORT</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Available supports:</font>
<br/> - CAS by LANCER called in by JTAC
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>LOGISTICS</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Maintenance:</font>
<br/> - RRR is available at the airfield
<br/> - Available rearm points: 2x
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Transportation:</font>
<br/> - Troop transport: Patgb 360 (2x)
<br/> - JTAC vehicle: Tgb 16 (1x)
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Available supplies:</font>
<br/> - Additional supplies can be found in the transport vehicles
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>SIGNALS</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Radio frequencies</font>
<br/> - LR ch. 69 - Infantry net (1 SEC, 2 SEC, JTAC Asst.)
<br/> - LR ch. 68 - CAS net (LANCER, JTAC Off.)
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Smoke signals</font>
<br/> - White: concealment
<br/> - Red: enemy positions
<br/> - Blue: friendly positions
<br/>
ENDTAB;

NEWTAB("VI. Mission notes:")
<br/><font color='#21749c' size='18' face='PuristaBold'>RESPAWNS</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Player respawn</font>
<br/> - Unlimited, rolling respawns
<br/> - 3-minute timer
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Reinsertion</font>
<br/> - Using the Squad Rally Point system
<br/>
<br/> - The section leader or dep. section leader has to place down a Rally Pint using the ACE interaction menu, then players can TP to their squad's Rally Point by interacting with the flag pole at the base.
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Asset respawn</font>
<br/> - No asset respawn is available
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>END CONDITIONS</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Success</font>
<br/> - Major success: all 3 objectives completed
<br/> - Minor success: 2 of 3 objectives completed
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Failure</font>
<br/> - Minor failure: 1 of 3 objectives completed
<br/> - Major failure: no objective completed / high civilian casualty / 80% of the platoon is dead at any point
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>RESTRICTIONS</font>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Use of enemy weapons</font>
<br/> - Only meds and throwables can be looted
<br/>
<br/><font color='#21749c' size='16' face='PuristaSemiBold'>Use of enemy vehicles</font>
<br/> - Only transport vehicles can be taken
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>JIP</font>
<br/> - Available, 3-minute timer
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>Mission maker's notes</font>
<br/> - The mission has no scripted ending. The Section leader and Dep. Section Leader of 1 SECTION can end the mission anytime by calling Tactical Withdrawal.
<br/> - If you are suffering from low framerate, you can disable the snowfall by using the ACE self-interaction menu.
<br/>
<br/>
<br/><font color='#21749c' size='18' face='PuristaBold'>Mission made by</font>
<br/> - Malbryn
ENDTAB;
