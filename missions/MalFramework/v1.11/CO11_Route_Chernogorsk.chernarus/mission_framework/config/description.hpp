...
briefingName = "[CO-11] Route Chernogorsk";  // Mission name on the mission selection screen (use [CO-XX] or [TVT-XX] where XX is the max. player number)
overviewText = "CDF units attept to regain control of the city of Chernagorsk. // Assets: 1x BMP-2K / T-72B (callsign: NOMAD) // Respawns: unlimited, rolling // Time limit: 120 minutes // Staging: enabled // JIP: enabled";  // Mission info on the mission selection screen (e.g. mission assets, factions, time limit etc.)

// Loading screen settings
onLoadName = "Route Chernogorsk";  // Name of the operation on the loading screen (you can drop the [CO-XX] part here)
