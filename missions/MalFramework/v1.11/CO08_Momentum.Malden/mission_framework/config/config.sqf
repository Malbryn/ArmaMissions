...
EGVAR(end_conditions,extMarker) = "mrk_extraction";  // Name of the extraction marker
...
EGVAR(end_conditions,playerThreshold) = 100;  // Percentage of the alive players that have to be in the extraction zone

// Task threshold (used by Time limit, Extraction and Tactical Withdrawal)
GVARMAIN(taskThreshold) = 100;  // Percentage of the tasks that have to be completed

// Respawn rules
GVARMAIN(respawnTimer) = 2 MINUTES;  // Respawn timer in seconds
...
EGVAR(ao_limit,timerLand) = 20;  // Timer for any land based vehicle/unit (-1: disabled)
EGVAR(ao_limit,timerAir) = 20;  // Timer for any air vehicle (-1: disabled)
EGVAR(ao_limit,aoMarkerAll) = "mrk_aoLimit";  // AO limit marker for every player
...
EGVAR(intro_text,title) = "MOMENTUM";  // Title
EGVAR(intro_text,date) = "2021-06-26 18:30";  // Date
EGVAR(intro_text,location) = "6km South of Malden International, Malden";  // Location
...
EGVAR(map_cover,aoMarker) = "mrk_aoLimit";  // Name of the AO marker
