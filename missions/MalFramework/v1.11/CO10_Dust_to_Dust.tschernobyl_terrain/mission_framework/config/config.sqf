...
EGVAR(end_conditions,timeLimitCheckEnabled) = false;  // Enable time limit check for players
...
EGVAR(respawn,timer) = 90;  // Respawn timer in seconds
...
EGVAR(respawn,removePlayerCorpses) = false;  // Remove the players' corpses upon respawn
...
EGVAR(ao_limit,aoMarkerBlufor) = "mrk_aoLimitAll";  // AO limit marker for BLUFOR players
EGVAR(ao_limit,aoMarkerRedfor) = "mrk_aoLimitAll";  // AO limit marker for REDFOR players
...
EGVAR(cbrn,timeLimit) = 10;  // The time limit in seconds after which the player (with insufficient protection) starts taking damage
EGVAR(cbrn,protectiveMasks) = ["CAU_G_CBRN_s10_blk", "CAU_G_CBRN_m04_hoodless", "SE_GP5", "SE_Respirator_Goggles", "SCE_G_M40_2"];  // Masks that provide protection (LVL 1)
...
EGVAR(intro_text,title) = "DUST TO DUST";  // Title
EGVAR(intro_text,date) = "1989-11-07 17:00";  // Date
EGVAR(intro_text,location) = "CHERNOBYL EXCLUSION ZONE, UKRAINIAN SSR";  // Location
