...
EGVAR(end_conditions,timeLimit) = 100 MINUTES;  // Mission time limit in seconds
...
EGVAR(ai_skills,skillSet) = [
    ...
    0.4   // Spotting Time (Higher = Faster)
];
...
EGVAR(ao_limit,timerAir) = 30;  // Timer for any air vehicle (-1: disabled)
...
EGVAR(intro_text,title) = "MORNING STAR";  // Title
EGVAR(intro_text,date) = "2019-08-12 0500";  // Date
EGVAR(intro_text,location) = "USS Liberty, Pyrgos Bay, Altis";  // Location
...
EGVAR(supply_drop,supplyDropPlane) = "RHS_C130J_Cargo";  // Plane or helicopter used for the supply drop
...
EGVAR(unit_tracking,refreshInterval) = 3;  // Refresh rate in seconds (min. value = 1)
