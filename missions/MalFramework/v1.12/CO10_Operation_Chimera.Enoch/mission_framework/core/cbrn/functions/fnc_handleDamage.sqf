...
private _newbloodVolume = _currentbloodVolume - (0.008 * (_damage / 5));  // Damage coeff. changed to 5 from 10 in this mission

SETVAR(_unit,ace_medical_bloodVolume,_newbloodVolume);

// Pain
private _currentPainLevel = GETVAR(_unit,ace_medical_pain,0);
private _newPainLevel = _currentPainLevel + (0.005 * (_damage / 5));  // Damage coeff. changed to 5 from 10 in this mission
