...
EGVAR(respawn,timer) = 120;  // Respawn timer in seconds
...
EGVAR(ao_limit,timerLand) = 30;  // Timer for any land based vehicle/unit (-1: disabled)
...
EGVAR(cbrn,protectiveMasks) = ["M40_Gas_mask_nbc_v4_d"];  // Masks that provide protection (LVL 1)
...
EGVAR(gear,loadoutHash) = [  // Key-value pairs (STRING - ARRAY OF STRINGS) for the alternative loadouts (Coop & TvT)
    ["LEADERSHIP - XM8", ["LEADERSHIP - AUG"]],
    ["MEDIC - XM8", ["MEDIC - AUG"]],
    ["AUTORIFLEMAN - XM8", ["AUTORIFLEMAN - M249"]],
    ["RIFLEMAN - XM8", ["RIFLEMAN - AUG", "BREACHER", "POINTMAN", "MARKSMAN"]]
];
...
EGVAR(intro_text,title) = "OPERATION CHIMERA";  // Title
EGVAR(intro_text,date) = "2020-06-26 19:45";  // Date
EGVAR(intro_text,location) = "NEAR NADBÓR, LIVONIA";  // Location
