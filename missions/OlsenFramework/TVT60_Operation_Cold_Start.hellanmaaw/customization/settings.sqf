...
if (isServer) then {
	...
	setViewDistance 1500;  // View distance for the server (the ai's)
	...
	[west, "US", "player"] call FNC_AddTeam;  // Adds a player team called USMC on side west
	[east, "SOV", "player"] call FNC_AddTeam;  // Adds a player team called VDV on side east
